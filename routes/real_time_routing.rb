...
class MyApp < Sinatra::Application
  ...
  post '/real_time_routing/destination_campaign_id.*' do
    ...
    format, request_payload = parser(request.path)
    prepend_to_hooklog_txt ip,method,format,request_payload
    
    destination_campaign_ids = request_payload['ids']
    destination_campaign_id = destination_campaign_ids.sample
    
    builder(format,"destination_campaign_id",destination_campaign_id,ip)
  end

  post '/real_time_routing/destination_campaign_id_from_network.*' do
    ip, method = basic_request_information  
    format, request_payload = request_parser(request.path)
    prepend_to_hooklog_txt ip,method,format,request_payload
    
    destination_campaign_ids = request_payload['ids']
    destination_campaign_id_from_network = destination_campaign_ids.sample
    
    response_builder(request_payload['format'],"destination_campaign_id_from_network",destination_campaign_id_from_network,ip)
  end
  ...
  post '/real_time_routing/destination_phone_number.*' do
    ...
    response_builder(request_payload['format'],"destination_phone_number",destination_phone_number,ip)
  end
